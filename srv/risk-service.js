...
module.exports = cds.service.impl(async function () {
    ...
    this.after("READ", Risks, async (data) => {
        ...
        console.log(await SELECT.from (Mitigations, risk.miti));
    ...
    this.before("READ", Risks, async (data, next) => {
        if (data.query.SELECT.orderBy){
            data.query.SELECT.orderBy[0].sort = "desc";
        }
    });
    ...
    this.on("getCountOfRisksByOwner", ({data:{owner}}) => {
        const risks = SELECT.from (Risks, {owner: owner});
        return risks.length;
    });

    this.on("RisePriority", ({data:{ID}}) => {
        return "Priority was rised";
    });
